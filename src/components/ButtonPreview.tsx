...
import type { ButtonTemplate } from '@/types'
...
export function ButtonPreview({ template, username }: ButtonPreviewProps) {
  const isDark = useResolvedTheme() === 'dark'
  const displayName = username || 'instagram_user'
  const initial = displayName[0]?.toUpperCase() || 'U'
  const visibleButtons = template.buttons.filter(b => b.title)
  const hasBody = template.text.trim().length > 0
  ...
  return (
    <div className={`rounded-2xl overflow-hidden border flex flex-col h-full ${isDark ? 'bg-zinc-950 border-zinc-800' : 'bg-white border-zinc-200'}`}>
      ...
      {/* Chat area */}
      <div className={`flex-1 p-4 space-y-2 overflow-y-auto min-h-[180px] ${isDark ? 'bg-zinc-900' : 'bg-zinc-50'}`}>
        {hasBody && (
          <div className="flex justify-end">
            <div
              className={`max-w-[85%] rounded-2xl rounded-br-sm overflow-hidden shadow-sm ${isDark ? 'bg-zinc-800 border border-zinc-700' : 'bg-white border border-zinc-200'}`}
            >
              <div className={`px-3.5 py-2 text-sm whitespace-pre-wrap break-words ${isDark ? 'text-zinc-100' : 'text-zinc-900'}`}>
                {template.text}
              </div>
              {visibleButtons.length > 0 && (
                <div className={`divide-y ${isDark ? 'divide-zinc-700 border-t border-zinc-700' : 'divide-zinc-200 border-t border-zinc-200'}`}>
                  {visibleButtons.map((btn, i) => (
                    <div
                      key={i}
                      className={`text-center text-[13px] font-medium py-2 text-[#3797F0] ${isDark ? 'bg-zinc-800 hover:bg-zinc-700' : 'bg-white hover:bg-zinc-50'}`}
                    >
                      {btn.title}
                    </div>
                  ))}
        ...
        {!hasBody && (
          <div className={`flex items-center justify-center h-24 text-xs ${isDark ? 'text-zinc-600' : 'text-zinc-400'}`}>
            Add a message body to see a preview
          </div>
        )}
      </div>
